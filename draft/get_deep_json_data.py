...
TARGET_URL = "https://www.huodongjia.com/event-327567164.html?json=1"
...
def get_event_cat_info_v3():
    l = [i['name'] for i in req_dict.get('event').get('event_cat_info')]
...
def get_event_schedule_v3():
    """
    can use, xiaoma improve
    :return:
    """
    event_info = req_dict.get('event')
    properties = event_info.get("properties", [])
    if not properties:
        return u""
    # 会议日程
    get_schedule4properties = lambda propertie: u"</br>".join(
        [u"<span>{0}<br/></span>{1}".format(child.get(u"name", ""), child.get(u"value", "")) for child in
         propertie.get(u"children")]) if u"children" in propertie else propertie.get("value", "")
    event_schedule = u"".join(
        [get_schedule4properties(propertie) for propertie in properties if u"日程" in propertie.get("name", "")])
    return event_schedule
...
if __name__ == '__main__':
    ...
    # print(get_event_cat_info_v3())


    # print(type(get_event_cat_info()))
    # print(get_event_schedule_v3())
