...
"""
需求：
1.每个账号每天发5篇，无需换IP发布，但需清除cookie，发布间隔3分钟。
2.发帖内容按规定格式提交
18683715921 不可用
"""
...
class AutoSender(object):
    ...
    def __init__(self, username="15281005385", password="YourPassword"):
    ...
    def send_article(self):
        ...
        # valid_url = self.check_valid_url(link_list)

        i = 0    # 单账号发贴初始值
        for valid_url in link_list:
            while i <= self.send_times:
                valid_json_url = self.domain_url + valid_url + "?json=1"
    ...
    def publish_article(self, valid_url):
        ...
        if req.status_code == 200:
            ...
            log.close()
            print('event_id has written in publish.log')
            return True
        else:
    ...
    def check_valid_url(self, link_list):
        ...
        for target_url in link_list:
            ...
            if target_url in check_list:
                ...
                target_url = self.domain_url + target_url + "?json=1"
                return target_url
    ...
    def get_event_url_from_cat_url(self):
        ...
        link_list = set(link_list)  # 页面上匹配的link
        return link_list


if __name__ == '__main__':
    auto = AutoSender(username='18108061758')
    auto.login_platform()
    auto.send_article()
    auto.clear_account_cache()

    auto = AutoSender(username='18782291154')
    auto.login_platform()
    auto.send_article()
    auto.clear_account_cache()
